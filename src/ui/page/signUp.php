  ...
  if (empty($username)) {
    ...
    if (!empty($users)) {
      ...
      if (in_array(strtolower($username), $users_array["username"])) {
        ...
        $is_error = false;
        $db->addUser($id, $username, $email, $phone, $password, "customer");
        $user_info = [
          "id" => $id,
          "username" => $username,
          "email" => $email,
...
<body class="bg-[#02081b] relative min-h-screen min-w-[97vw] flex justify-center items-center overflow-hidden text-white">
  <form autocomplete="off" action="./signUp.php" method="post" class="relative z-10 bg-[#02081b] px-8 md:px-9 py-9 drop-shadow-[0_0_5px_rgba(255,255,255,0.15)] text-white flex flex-col gap-2 md:gap-3 w-full rounded-lg max-w-[350px] sm:max-w-[400px] md:max-w-[450px] transition-all duration-300 hover:drop-shadow-[0_0_5px_rgba(255,255,255,0.25)]">
    <h1 class="uppercase text-xl tracking-wider font-bold">sign up</h1>

    <div class="px-2 flex flex-col">
      <label for="username" class="capitalize tracking-wider font-semibold">username:</label>
      <input autofocus id="username" name="username" type="text" class="transition-all duration-500 border-2 py-2 border-white bg-[#02081b] px-5 focus:outline-none mt-2 rounded-xl focus:border-t-none focus:border-x-transparent focus:rounded-none focus:border-t-transparent" placeholder="Enter your name" value="<?= $username ?>">
    </div>

    <div class="px-2 flex flex-col">
      <label for="email" class="capitalize tracking-wider font-semibold">email:</label>
      <input id="email" name="email" type="email" class="transition-all duration-500 border-2 py-2 border-white bg-[#02081b] px-5 focus:outline-none mt-2 rounded-xl focus:border-t-none focus:border-x-transparent focus:rounded-none focus:border-t-transparent" placeholder="Enter your email address" value="<?= $email ?>">
    </div>

    <div class="px-2 flex flex-col">
      <label for="phone" class="capitalize tracking-wider font-semibold">phone number:</label>
      <input id="phone" name="phone" type="number" class="transition-all duration-500 border-2 py-2 border-white bg-[#02081b] px-5 focus:outline-none mt-2 rounded-xl focus:border-t-none focus:border-x-transparent focus:rounded-none focus:border-t-transparent" placeholder="Enter your phone number" value="<?= $phone ?>">
    </div>
    ...
    <div class="px-2 flex flex-col">
      ...
      <input id="rePpassword" name="conPass" type="password" class="transition-all duration-500 border-2 py-2 border-white bg-[#02081b] px-5 focus:outline-none mt-2 rounded-xl focus:border-t-none focus:border-x-transparent focus:rounded-none focus:border-t-transparent" placeholder="Re-Enter your password" value="<?= $conPass ?>">
    </div>
    ...
    <div class="flex flex-col items-center ">
      ...
      <p class="-mt-1">or</p>
      <p class="capitalize -mt-1">register as a <a class="underline hover:text-purple-600" href="./doctorRegister.php">doctor</a></p>
    </div>
  </form>
